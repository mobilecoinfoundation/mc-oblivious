...
use mc_oblivious_ram::{PathORAM, PathOramEvict, CircuitOramNonobliviousEvict};
...
/// Creator for NonObliviousCircuitOram based on 4096-sized blocks of storage and bucket size
/// (Z) of 4, and the insecure position map implementation.
/// This is used to determine calibrate circuit oram
pub struct InsecureNonObliviousCircuitORAM4096Z4Creator<R, SC: ORAMStorageCreator<U4096, U64>, const N: usize>
where
    R: RngCore + CryptoRng + 'static,
{
    _sc: PhantomData<fn() -> SC>,
...
impl<R, SC: ORAMStorageCreator<U4096, U64>, const N: usize> ORAMCreator<U1024, R>
    for InsecureNonObliviousCircuitORAM4096Z4Creator<R, SC, N>
where
    R: RngCore + CryptoRng + Send + Sync + 'static,
    SC: ORAMStorageCreator<U4096, U64>,
{
